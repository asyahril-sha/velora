...
from core.role_session import get_session_manager, update_session_activity
# ======================================================
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ========== TAMBAHKAN: Cek mode dari session manager ==========
    session_manager = get_session_manager()
    session_mode = session_manager.get_mode(user_id)
    active_role = session_manager.get_active_role(user_id)
    
    if session_mode.value == "role" and active_role != "nova":
        logger.info(f"🎭 [SESSION] User {user_id} in ROLE mode, active: {active_role}")
    # ================================================================
    
    # Cek mode user dari old system (untuk kompatibilitas)
