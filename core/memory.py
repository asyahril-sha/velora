...
- Memory filtering berdasarkan role awareness
...
class MemoryEvent:
    ...
    kejadian: str
    detail: str
    source: str  # nova, user, ipar, pelakor, dll
    role_id: str
...
class LongTermMemory:
    ...
    tipe: str  # kebiasaan_mas, momen_penting, janji, rencana
    judul: str
    konten: str
    perasaan: str = ""
...
class MemoryManager:
    ...
    Semua role mengakses memory melalui manager ini.
    """
    ...
    def add_event(self, 
        ...
        logger.debug(f"📝 Event added: [{source}] {kejadian[:50]}")
    ...
    def add_long_term_memory(self,
        ...
        if len(self.long_term[role_id]) > 100:
            # Hapus yang paling tidak penting
            self.long_term[role_id].sort(key=lambda x: x.importance)
            self.long_term[role_id].pop(0)
        
        logger.info(f"📝 Long-term memory added: {role_id} - {tipe}: {judul[:50]}")
            ...
            if awareness:
                ...
                for event in recent:
                    ...
                    if awareness.awareness_level == AwarenessLevel.LIMITED:
                        if event.source == "user" or event.tags:
    ...
    def get_short_term_text(self, count: int = 10, role_id: str = None) -> str:
        ...
        lines = [
            "═══════════════════════════════════════════════════════════════",
            "⚠️ KEJADIAN TERAKHIR (WAJIB DIPERHATIKAN! JANGAN LUPA!):",
            "═══════════════════════════════════════════════════════════════"
        ]
    ...
    def get_long_term_text(self, role_id: str = "nova") -> str:
        ...
        if promises:
            pending = [p for p in promises if p.get('status', 'pending') == 'pending']
            if pending:
    ...
    def get_context_for_role(self, role_id: str, include_timeline: bool = True) -> str:
        ...
        if knowledge:
            parts.append(f"\n{'-'*70}\nFAKTA YANG KAMU KETAHUI:\n" + "\n".join([f"- {f}" for f in knowledge[:10]]))
    ...
    def propagate_to_other_roles(self, event: MemoryEvent, exclude_role: str = None) -> None:
        ...
        for role_id, awareness in self.world.role_awareness.items():
            ...
            if can_know:
                ...
                if event.drama_impact > 10 or "important" in event.tags:
                    self.add_long_term_memory(
                        tipe="momen_penting",
                        judul=event.kejadian[:50],
                        konten=event.detail[:100],
                        role_id=role_id,
                        importance=int(event.drama_impact / 10) if event.drama_impact > 0 else 5
                    )
    ...
    def cleanup_old_memories(self, max_age_days: int = 30) -> None:
        ...
        # Bersihkan short-term (sudah sliding window)
        # Tidak perlu, sudah otomatis
