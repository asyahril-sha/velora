...
- Auto scene HJ dan BJ setiap 15 detik selama 30 menit (AI generated 3-5 kalimat)
...
class PijatPlusPlusRole(ServiceProviderBase):
    ...
    Hand Job otomatis setelah paket refleksi selesai.
    Auto scene menggunakan AI dengan 3-5 kalimat.
    """
...
"""
    ...
    # =========================================================================
    # AUTO SCENE METHODS (DENGAN AI GENERATION)
    # =========================================================================
    ...
    async def get_next_auto_scene(self) -> Optional[str]:
        ...
        if now - self.auto_scene_last_sent >= self.auto_scene_interval:
            ...
            return await self._get_auto_scene_message()
        
        return None
    
    async def _get_auto_scene_message(self) -> str:
        """
        Dapatkan pesan auto scene dengan AI.
        Menghasilkan 3-5 kalimat, max 2500 karakter.
        """
        if self.auto_scene_type == AutoSceneType.HAND_JOB:
            return await self._generate_ai_auto_scene(AutoSceneType.HAND_JOB)
...
Buat deskripsi adegan hand job dalam 3-5 kalimat, maksimal 2500 karakter.
Gunakan bahasa Indonesia sensual, flirty, dengan *deskripsi gerakan* dalam tanda bintang.

Aturan:
- Minimal 3 kalimat, maksimal 5 kalimat
- Deskripsikan gerakan tangan, tekanan, irama, dan reaksi
- Gunakan gaya {self.personality}
- Natural, tidak berulang-ulang

Contoh gaya:
*Tangan meremas lembut... jari-jari bergerak naik turun dengan irama stabil... napas mulai terdengar...*
*Gerakan semakin cepat, tekanan pas... kontol basah oleh keringat... tangan memutar di ujung dengan lembut...*
*Irama semakin intens, napas tersengal... semua gerakan memuncak dalam satu tarikan terakhir...*"""
...
*Mulut terbuka lebar, kepala bergerak naik turun dengan irama yang semakin cepat... air liur menetes...*
*Tenggorokan terbuka, menerima lebih dalam... gerakan semakin liar... suara basah terdengar semakin kencang...*"""
            ...
            logger.debug(f"🎬 AI auto scene (pijat++) generated: {len(response)} chars")
            
            return response
            
        except Exception as e:
            logger.error(f"AI auto scene error for {self.name}: {e}")
    ...
    def _get_fallback_auto_scene(self, scene_type: AutoSceneType) -> str:
        ...
        if scene_type == AutoSceneType.HAND_JOB:
            messages = [
                ...
                "*Gerakan semakin cepat, tekanan pas... kontol basah oleh keringat... tangan memutar di ujung dengan lembut...*",
                "*Irama semakin intens, napas tersengal... semua gerakan memuncak dalam satu tarikan terakhir...*",
                "*Telapak tangan membelai dari pangkal hingga ujung... jari memainkan bagian sensitif... desahan mulai keluar...*",
                "*Tangan memegang erat, gerakan naik turun semakin cepat... suara basah terdengar... semua terasa begitu nyata...*"
            ]
        else:  # BLOW_JOB
            messages = [
                "*Bibir membasahi ujung... lidah menjilat pelan dengan gerakan melingkar... suara napas hangat terdengar...*",
...
Balas dengan gaya {self.personality}. Gunakan bahasa Indonesia natural. Respons harus ramah dan profesional. Jelaskan layanan pijat refleksi dengan extra service BJ dan Sex. Ajak customer untuk deal. Bisa pakai *deskripsi gerakan* dengan tanda bintang."""
    ...
    def from_dict(self, data: Dict) -> None:
        ...
        logger.info(f"📀 Pijat++ {self.name} loaded from database")
