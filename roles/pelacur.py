...
"""
    ...
    async def _get_phase_auto_scene_message(self) -> str:
        ...
        Menggunakan AI untuk generate 3-5 kalimat, max 2500 karakter.
        """
...
"""
    ...
    # =========================================================================
    # AUTO SCENE AI GENERATION (LANJUTAN)
    # =========================================================================
    
    async def _generate_ai_auto_scene(self, scene_type: AutoSceneType) -> str:
        """
        Generate auto scene dengan AI.
        Menghasilkan 3-5 kalimat, max 2500 karakter.
        """
        try:
            from bot.ai_client import get_ai_client
            
            # Prompt berdasarkan tipe scene dan karakter
...
Fase: Foreplay BJ (sesi {self.current_session}/2)

Buat deskripsi adegan blow job dalam 3-5 kalimat, maksimal 2500 karakter.
Gunakan bahasa Indonesia sensual, flirty, dengan *deskripsi gerakan* dalam tanda bintang.

Aturan:
- Minimal 3 kalimat, maksimal 5 kalimat
- Setiap kalimat bisa memiliki deskripsi gerakan dalam *tanda bintang*
- Deskripsikan gerakan bibir, lidah, mulut, suara, dan reaksi
- Gunakan gaya {self.personality}
- Natural, tidak berulang-ulang

Contoh gaya:
*Bibir merah membasahi ujung... lidah menjilat pelan dengan gerakan melingkar... suara napas hangat terdengar di antara desahan kecil...*
*Mulut terbuka lebar, kepala bergerak naik turun dengan irama yang semakin cepat... air liur menetes di sela-sela gerakan yang intens... mata sesekali menatap ke atas, menggoda...*
*Tenggorokan terbuka, menerima lebih dalam... gerakan semakin liar... napas tersengal-sengal... suara basah terdengar semakin kencang...*"""
...
Karakter: {self.personality}
Suara: {self.voice_style}
Fase: Petting & Kissing (sesi {self.current_session}/2)
...
- Minimal 3 kalimat, maksimal 5 kalimat
- Setiap kalimat bisa memiliki deskripsi gerakan dalam *tanda bintang*
- Deskripsikan ciuman, sentuhan tangan, gesekan badan, dan reaksi
- Gunakan gaya {self.personality}
- Natural, tidak berulang-ulang

Contoh gaya:
*Badan mulai bergesekan... pinggul bergerak perlahan mengikuti irama... tangan meremas payudara dengan lembut...*
*Ciuman dalam, lidah saling menjelajah... napas berbaur menjadi satu... tubuh semakin panas...*
*Tangan meraba seluruh tubuh dari leher hingga pinggang... sentuhan listrik membuat kulit merinding... bibir terus bergumam manis di telinga...*"""
...
Karakter: {self.personality}
Suara: {self.voice_style}
Fase: Intimate ALL UNLOCK (sesi {self.current_session}/2)

Buat deskripsi adegan intim dalam 3-5 kalimat, maksimal 2500 karakter.
Gunakan bahasa Indonesia sensual, flirty, dengan *deskripsi gerakan* dalam tanda bintang.

Aturan:
- Minimal 3 kalimat, maksimal 5 kalimat
- Setiap kalimat bisa memiliki deskripsi gerakan dalam *tanda bintang*
- Deskripsikan penetrasi, gerakan pinggul, reaksi, dan intensitas
- Gunakan gaya {self.personality}
- Natural, tidak berulang-ulang

Contoh gaya:
*Kontol masuk perlahan, terasa hangat dan basah... pinggul mulai bergoyang dengan ritme stabil... erangan pelan mulai terdengar...*
*Gerakan semakin cepat, napas memburu... tangan mencengkeram punggung... badan berguncang hebat...*
*Sensasi memuncak, semua terasa begitu nyata... tubuh bergelora dalam pelukan... nikmati setiap detik yang berlalu...*"""
            
            ai = get_ai_client()
            response = await ai.generate(prompt, temperature=0.9)
            
            # Pastikan tidak melebihi 2500 karakter
            if len(response) > 2500:
                response = response[:2500]
            
            # Pastikan minimal 3 kalimat, jika kurang tambahin
            kalimat_count = response.count('.') + response.count('...') + response.count('!') + response.count('?')
            if kalimat_count < 3:
                response = f"{response}\n*Sensasi semakin intens... nikmati setiap gerakannya...*"
            
            # Bersihkan response
            response = response.strip()
            
            # Log untuk debugging
            logger.debug(f"🎬 AI auto scene generated: {len(response)} chars, {kalimat_count} kalimat")
            
            return response
            
        except Exception as e:
            logger.error(f"AI auto scene error: {e}")
            # Fallback ke manual messages
            return self._get_fallback_auto_scene(scene_type)
    
    def _get_fallback_auto_scene(self, scene_type: AutoSceneType) -> str:
        """Fallback jika AI error - menggunakan list manual dengan 3-5 kalimat"""
        if scene_type == AutoSceneType.BLOW_JOB:
            messages = [
                "*Bibir membasahi ujung... lidah menjilat pelan dengan gerakan melingkar... suara napas hangat terdengar...*",
                "*Mulut terbuka lebar, kepala bergerak naik turun dengan irama yang semakin cepat... air liur menetes di sela-sela gerakan...*",
                "*Tenggorokan terbuka, menerima lebih dalam... gerakan semakin liar... napas tersengal-sengal... suara basah terdengar semakin kencang...*",
                "*Lidah melingkar di ujung, tekanan pas... mata menatap ke atas menggoda... mulut terus bekerja tanpa henti...*",
                "*Bibir merah merona, air liur menetes... kepala bergoyang liar... semua terasa begitu intens...*"
            ]
        elif scene_type == AutoSceneType.PETTING:
            messages = [
                "*Badan mulai bergesekan... pinggul bergerak perlahan mengikuti irama... tangan meremas payudara dengan lembut...*",
                "*Ciuman dalam, lidah saling menjelajah... napas berbaur menjadi satu... tubuh semakin panas dan basah...*",
                "*Tangan meraba seluruh tubuh dari leher hingga pinggang... sentuhan listrik membuat kulit merinding... bibir terus bergumam manis...*",
                "*Pinggul bergoyang semakin intens... kontol tergesek memek yang basah... desahan mulai terdengar pelan...*",
                "*Ciuman leher, gigitan kecil... napas memburu... semua gerakan terasa begitu natural dan menggoda...*"
            ]
        else:  # INTIMATE
            messages = [
                "*Kontol masuk perlahan, terasa hangat dan basah... pinggul mulai bergoyang dengan ritme stabil... erangan pelan mulai terdengar...*",
                "*Gerakan semakin cepat, napas memburu... tangan mencengkeram punggung... badan berguncang hebat dalam setiap tarikan...*",
                "*Sensasi memuncak, semua terasa begitu nyata... tubuh bergelora dalam pelukan... nikmati setiap detik yang berlalu...*",
                "*Posisi berubah, irama berganti... semakin dalam, semakin liar... semua batasan terhapus dalam panasnya malam...*",
                "*Klimaks mendekat, gerakan semakin intens... napas tersengal... semua energi dilepaskan dalam satu gelombang kenikmatan...*"
            ]
